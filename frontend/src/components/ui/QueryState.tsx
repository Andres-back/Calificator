...
export function QueryLoading({ count = 3, className = '', label }: { count?: number; className?: string; label?: string }) {
  return (
    <div className={`grid gap-3 ${className}`}>
      {label && <p className="px-2 text-xs font-semibold text-muted">{label}</p>}
...
export function QueryError({
  ...
  const statusCode = code ?? (error as any)?.response?.status ?? (error as any)?.status;
  const displayMessage = message ?? description ?? friendlyMessage(statusCode);
  const displayTitle = title ?? 'Algo salió mal';

  return (
    <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-rose-200 bg-surface px-6 py-14 text-center dark:border-rose-500/20">
      ...
      {statusCode && (
        <span className="mt-3 rounded-full bg-surface-2 px-2.5 py-0.5 text-[10px] font-semibold text-muted">
          Error {statusCode}
        </span>
      )}
