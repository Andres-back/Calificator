"""Agentes de calificación para XCalificator.

Cada agente es un wrapper tipado sobre un modelo de OpenCode (o proveedor alternativo).
Siguen una interfaz común: reciben un contexto de calificación, devuelven un resultado estructurado.
"""
...
async def vision_agent(
    ...
    Args:
        ctx: Contexto con la imagen.
        model: Modelo de visión (mimo-v2.5, qwen3.6-plus, etc.)
        client: Cliente reutilizable (opcional).
    """
...
async def grader_agent(
    ...
    Args:
        ctx: Contexto con blueprint, RAG y respuesta.
        model: Modelo a usar (deepseek-v4-flash, qwen3.7-plus, etc.)
        multimodal: Si True, envía la imagen directamente al modelo.
        client: Cliente reutilizable (opcional).
    """
    ...
    try:
        ...
        if multimodal and ctx.image_bytes:
            # Qwen recibe la imagen directo y califica todo él solo
            raw = await client.chat_multimodal(
                model=model,
                text=prompt,
                image_bytes=ctx.image_bytes,
        ...
        # Verificar razonamiento de qwen (viene en reasoning_content)
        ...
        logger.info(
            "Grader agent OK: modelo=%s %dms nota=%.2f confianza=%.2f",
            model, ms, result.nota_sugerida, result.confianza,
        )
        return result
...
async def comparator_agent(
    ...
    Args:
        grading_a: Resultado del primer calificador.
        grading_b: Resultado del segundo calificador.
        umbral: Diferencia máxima para considerar consenso automático.
        model: Modelo para el análisis comparativo.
    """
    diff = abs(grading_a.nota_sugerida - grading_b.nota_sugerida)

    # Si no hay error en ninguno y las notas están cerca, promedio directo (rápido)
    if not grading_a.error and not grading_b.error and diff < umbral:
        ...
        logger.info(
            "Comparator: consenso automatico diff=%.2f nota=%.2f",
            diff, nota_final,
        )
        return AgentResult(
    ...
    # Si hay discrepancia o error, usar LLM para resolver
    client = OpenCodeClient()
    try:
        grading_a_str = json.dumps({
            "nota_sugerida": grading_a.nota_sugerida,
            "confianza": grading_a.confianza,
            "feedback": grading_a.feedback_estudiante[:300],
            "criterios": grading_a.criterios,
            "alertas": grading_a.alertas,
            "error": grading_a.error,
        }, ensure_ascii=False)

        grading_b_str = json.dumps({
            "nota_sugerida": grading_b.nota_sugerida,
            "confianza": grading_b.confianza,
            "feedback": grading_b.feedback_estudiante[:300],
            "criterios": grading_b.criterios,
            "alertas": grading_b.alertas,
        ...
        return AgentResult(
            nota_sugerida=nota_final,
            confianza=grading_a.confianza if not grading_a.error else grading_b.confianza,
        ...
        # Fallback: promedio simple si LLM falla
        nota_final = round((grading_a.nota_sugerida + grading_b.nota_sugerida) / 2, 2)
        return AgentResult(
            nota_sugerida=nota_final,
            confianza=0.3,
            feedback_estudiante=grading_a.feedback_estudiante or grading_b.feedback_estudiante,
            alertas=["Comparador no disponible, se usó promedio simple"],
            requiere_revision_docente=True,
            proveedor="comparator",
            modelo="fallback",
        )
