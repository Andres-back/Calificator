from sqlalchemy import Column, ForeignKey, Index, Integer, MetaData, Table, UniqueConstraint

from app.db.schema_ownership import (
    DEPRECATED_SQL_TABLES,
    EXTERNAL_PRESENTON_TABLES,
    SQL_MANAGED_TABLES,
...
def test_known_sql_and_external_tables_are_excluded_when_reflected() -> None:
    metadata = MetaData()
    for table_name in SQL_MANAGED_TABLES | DEPRECATED_SQL_TABLES | EXTERNAL_PRESENTON_TABLES:
...
def test_sql_managed_artifacts_are_preserved() -> None:
    ...
    attendance = Table("asistencia_registros", metadata, Column("estudiante_id", Integer))
    legacy_index = Index("idx_asistencia_estudiante", attendance.c.estudiante_id)
    assert include_schema_object(legacy_index, legacy_index.name, "index", True, None) is False
    ...
    constraint = next(item for item in users.constraints if item.name == "uq_users_email")
    assert include_schema_object(
        constraint,
        constraint.name,
        "unique_constraint",
